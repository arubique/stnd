...
def get_default_log_file_path():
    return os.path.join(
        get_project_root_path(), "tmp", "tmp_log_for_run_from_csv.out"
    )
        ...
        if len(starmap_args_for_row_processing):
            ...
            with mp.Pool(pool_size) as pool:
                ...
                if len(shared_rows_to_run):
                    assert 2 * len(shared_rows_to_run) == len(
                        shared_csv_updates
                    )

                    concurrent_log_func = retrier_factory()(
                        log_csv_for_concurrent
                    )
                    ...
                    optionally_make_parent_dir(log_file_path)
                    logger.log(
                    ...
                    pool.starmap(submit_job, starmap_args_for_job_submitting)
                    upload_csv = True
...
def fetch_default_config_path(path, logger):
    if FILES_URL in path:
        ...
        with NamedTemporaryFile("w+t", delete=False) as tmp_file:
            ...
            optionally_make_parent_dir(file_path)
...
def process_csv_row(
    ...
    if is_number(whether_to_run) and int(whether_to_run) != 0:
        replace_placeholders(csv_row, CURRENT_ROW_PLACEHOLDER, str(row_number))
        replace_placeholders(
            csv_row, CURRENT_WORKSHEET_PLACEHOLDER, worksheet_name
        )
...
def make_final_cmd_slurm(
    ...
    if USE_SRUN:
        ...
        final_cmd = 'srun {} sh -c "{}" &'.format(
            slurm_args_as_string, cmd_as_string
        )
    else:
...
def make_new_config(
    ...
    optionally_make_parent_dir(new_config_path)
...
def make_slurm_args_dict(csv_row, exp_name, log_file):
    ...
    optionally_make_parent_dir(all_slurm_args_dict["output"])
    optionally_make_parent_dir(all_slurm_args_dict["error"])
...
def extract_from_csv_row_by_prefix(csv_row, prefix, ignore_values):
    ...
    for key, value in csv_row.items():
        ...
        if (
            len(key) > prefix_len
            and prefix == key[:prefix_len]
            and not value in ignore_values
        ):
