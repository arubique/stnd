...
# local modules
from .utils import error_or_print, get_nested_attr, get_with_assert
...
class LazyModuleWrapper(ModuleType):
    def __init__(self, module_name):
        ...
        self.default_attrs = {"__spec__": spec, "__name__": spec.name}
...
def make_lazy_module(
    ...
    return _LazyModule(
        name, module_file, import_structure, module_spec, extra_objects
    )


def pop_all_modules_by_filter(filter_condition, filter_name=None, logger=None):
    imported_modules = sorted(
        list(filter(filter_condition, sys.modules.keys()))
    )
    if filter_name is None:
        filter_name = " AND ".join(
            set.intersection(
                *[
                    set(name.split(PACKAGE_SEPARATOR))
...
def import_from_string(string, reload=False, nested_attrs_depth=1):
    ...
    if len(nested_attrs) > 0:
        module = get_nested_attr(module, nested_attrs)
...
def make_from_class_ctor(from_class_config, pos_args_list=[]):
    ...
    importable_kwargs = update_enums_in_config(
        importable_kwargs, importable_kwargs.keys()
    )

    return class_ctor(*pos_args_list, **(kwargs | importable_kwargs))
    ...
    for enum in enums:
        if enum in config:
            ...
            config[enum] = import_from_string(
                config[enum], nested_attrs_depth=nested_attrs_depth
            )
    return config
