...
from stnd.utility.configs import EXP_NAME_CONFIG_KEY, get_config
...
def get_diff_with_unstaged_changes(repo):
    ...
    try:
        ...
        diff = repo.index.diff(
            None, create_patch=True
        )  # None indicates working tree vs. index
            ...
            try:
                ...
                with redneck_logger_context(
                    ...
                    if check_config is not None:
                        ...
                        check_config(
                            experiment_config, config_path, logger=logger
                        )
...
def define_env_vars():
    if SCRATCH_VAR_NAME not in os.environ and os.path.exists(SCRATCH_LOCAL):
        ...
        os.environ[SCRATCH_VAR_NAME] = os.path.join(
            SCRATCH_LOCAL, any_folder_of_user
        )
