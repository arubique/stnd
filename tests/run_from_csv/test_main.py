...
sys.path.insert(
    0, os.path.join(os.path.dirname(CUR_FOLDER))
)
...
def test_env():
    ...
    with tempfile.TemporaryDirectory(prefix=CUR_FOLDER) as temp_dir:
        ...
        parent_git_dir = os.path.join(os.path.dirname(os.path.dirname(CUR_FOLDER)), ".git")
...
def test_main_script(test_env):
    ...
    if conda_env != "None":
        assert os.path.exists(conda_env), f"Conda env {conda_env} does not exist"
        python_binary = os.path.join(conda_env, "bin", "python3.10")
        assert os.path.exists(python_binary), f"Python binary {python_binary} does not exist"
    else:
    ...
    assert set(canonical_df.columns) == set(df.columns), "CSV columns do not match canonical CSV"
    ...
    cols_to_compare = [col for col in canonical_df.columns if col not in UNIQUE_COLUMNS]
    for col in cols_to_compare:
        value_in_df = df[col].iloc[0]
        if isinstance(value_in_df, str):
            value_in_df = value_in_df.replace(CUR_FOLDER, ".")
        canonical_value = canonical_df[col].iloc[0]
        assert value_in_df == canonical_value, f"Column {col} does not match canonical CSV"
