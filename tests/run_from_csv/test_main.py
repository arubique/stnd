...
from stnd.run_from_csv.__main__ import (
    RUNNER_PLACEHOLDER,
    COMMA_PLACEHOLDER,
...
def test_make_task_cmd_skips_shell_init_for_prefixed_conda_env():
    conda_env = (
        f"{SKIP_SHELL_INIT_PREFIX}cd{SKIP_SHELL_INIT_PREFIX}/tmp"
        f"{SKIP_SHELL_INIT_PREFIX}{AMPERSAND_PLACEHOLDER}"
        f"{SKIP_SHELL_INIT_PREFIX}uv{SKIP_SHELL_INIT_PREFIX}run"
        f"{SKIP_SHELL_INIT_PREFIX}--flag={QUOTE_PLACEHOLDER}"
        f"a{COMMA_PLACEHOLDER}b{BACKSLASH_PLACEHOLDER}c{QUOTE_PLACEHOLDER}"
    )
    ...
    assert (
        cmd
        == 'cd /tmp && uv run --flag="a,b\\c" python /tmp/train.py --config_path /tmp/config.yaml'
    )
