...
# local imports
STND_ROOT = os.path.dirname(os.path.dirname(__file__))
...
def demo_experiment(
    ...
    if experiment_config["image"]["color"] == "red":
        channel = 0
    elif experiment_config["image"]["color"] == "green":
        channel = 1
    elif experiment_config["image"]["color"] == "blue":
        channel = 2
    else:
        raise ValueError(
            f"Invalid color: {experiment_config['image']['color']}"
        )
    ...
    for i in range(10):
        ...
        try:
            ...
            try_to_log_in_wandb(logger, wandb_stats_to_log, step=i)
        except ImportError:
